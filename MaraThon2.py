...
def get_gist_id(filename):
    ...
    try:
        resp = requests.get("https://api.github.com/gists", headers={"Authorization": f"token {st.secrets['github']['token']}"})
...
def load_data_from_gist(filename):
    ...
    try:
        resp = requests.get(f"https://api.github.com/gists/{gist_id}", headers={"Authorization": f"token {st.secrets['github']['token']}"})
        resp.raise_for_status()
...
def save_data_to_gist(filename, data):
    ...
    try:
        gist_id = get_gist_id(filename)
        payload = {"description": f"Storage for {filename}", "public": False, "files": {filename: {"content": json.dumps(data, ensure_ascii=False, indent=2)}}}
        if gist_id:
            requests.patch(f"https://api.github.com/gists/{gist_id}", json=payload, headers={"Authorization": f"token {st.secrets['github']['token']}"})
        else:
            requests.post("https://api.github.com/gists", json=payload, headers={"Authorization": f"token {st.secrets['github']['token']}"})
    except: pass
...
def load_config():
    ...
    if 'closures' not in config: config['closures'] = {}; changed = True
    if 'email_settings' not in config: config['email_settings'] = {"default_to": "", "default_subject": "Rozpis služieb", "default_body": "Dobrý deň,\nv prílohe rozpis.\nS pozdravom"}; changed = True
...
def migrate_homolova_to_vidulin(config):
    changed = False
    if "Homolova" in config["lekari"]: config["lekari"]["Vidulin"] = config["lekari"].pop("Homolova"); changed = True
    for amb_name, amb_data in config["ambulancie"].items():
        if isinstance(amb_data["priority"], list):
            if "Homolova" in amb_data["priority"]: amb_data["priority"] = ["Vidulin" if x == "Homolova" else x for x in amb_data["priority"]]; changed = True
        elif isinstance(amb_data["priority"], dict):
            for day_key, day_list in amb_data["priority"].items():
                if "Homolova" in day_list: amb_data["priority"][day_key] = ["Vidulin" if x == "Homolova" else x for x in day_list]; changed = True
...
def distribute_rooms(doctors_list, wolf_doc_name, previous_assignments=None, manual_core=None):
    ...
    if rt_help_doc and head_doc != rt_help_doc and rt_help_doc not in pure_workers: pure_workers.append(rt_help_doc)
    ...
    if num_workers >= 2 and head_doc in active_assignees and head_doc != rt_help_doc: active_assignees.remove(head_doc)
...
def generate_data_structure(config, absences, start_date, save_hist=True):
    ...
    for i in range(7):
        ...
        for amb_name in processing_order:
            ...
            if amb_name in closed_today: assigned_amb[amb_name] = "ZATVORENÉ"; continue
            amb_info = config['ambulancie'][amb_name]
            if day_name not in amb_info['dni']: assigned_amb[amb_name] = "---"; continue
            if amb_name == "Radio 2B" and "Martinka" not in available: assigned_amb[amb_name] = "ZATVORENÉ"; continue
        ...
        if "ODDELENIE (Celé)" in closed_today:
            ...
            for doc in all_doctors:
                ...
                if "Oddelenie" in config['lekari'][doc].get('moze', []): room_text_map[doc] = "ZATVORENÉ"
        else:
            ward_candidates = [d for d in available if "Oddelenie" in config['lekari'][d].get('moze', [])]
            if wolf_doc and wolf_doc not in ward_candidates and "Oddelenie" in config['lekari'].get(wolf_doc, {}).get('moze', []): ward_candidates.append(wolf_doc)
        ...
        for date_str in dates:
            ...
            for amb_name in ["Prijmova", "Velka dispenzarna", "Mala dispenzarna", "Radio 2A", "Radio 2B", "Chemo 8A", "Chemo 8B", "Chemo 8C", "Wolf"]:
                ...
                if val in ["NEOBSADENÉ", "???", ""] and amb_name not in closed_today and "ODDELENIE (Celé)" not in closed_today:
                    display_map = {"Radio 2A": "RT ambulancia", "Velka dispenzarna": "veľký dispenzár", "Mala dispenzarna": "malý dispenzár"}
...
def create_display_df(dates, data_grid, all_doctors, motto, config):
    ...
    sections = [("Konziliárna amb", ["Prijmova"]), ("RT ambulancie", ["Radio 2A", "Radio 2B"]), ("Chemo amb", ["Chemo 8A", "Chemo 8B", "Chemo 8C"]), ("Disp. Ambulancia", ["Velka dispenzarna", "Mala dispenzarna"]), ("RTG Terapia", ["Wolf"])]
...
def create_pdf_report(df, motto):
    ...
    style = TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.grey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('BOTTOMPADDING', (0, 0), (-1, 0), 12), ('BACKGROUND', (0, 1), (-1, -1), colors.white), ('GRID', (0, 0), (-1, -1), 1, colors.black), ('FONTSIZE', (0, 0), (-1, -1), 8), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE')])
...
def send_email_with_pdf(pdf_bytes, filename, to_email, subject, body):
    if "email" not in st.secrets: st.error("Chýbajú nastavenia emailu"); return False
    ...
    try:
        ...
        server.quit()
        return True
    except Exception as e: st.error(f"Chyba: {e}"); return False
...
if mode == "🚀 Generovať rozpis":
    ...
    with st.expander("📅 Výnimky a zatváranie"):
        ...
        if c_ex3.button("💾 Uložiť"):
            if 'closures' not in st.session_state.config: st.session_state.config['closures'] = {}
            if len(d_range) == 2:
                curr = d_range[0]
                cnt = 0
                while curr <= d_range[1]:
                    d_key = curr.strftime('%Y-%m-%d')
                    if selected_closures: st.session_state.config['closures'][d_key] = selected_closures
                    else:
                        if d_key in st.session_state.config['closures']: del st.session_state.config['closures'][d_key]
                    curr += timedelta(days=1)
                    cnt += 1
    ...
    if 'df_display' in st.session_state:
        ...
        with st.expander("📧 Odoslať emailom"):
            ...
            if st.button("📤 Odoslať PDF"):
                if send_email_with_pdf(pdf_data, f"{filename_base}.pdf", to_email, subject, body): st.success("Email odoslaný!")
        st.subheader("📄 Náhľad")
    ...
    if st.button("💾 Uložiť"):
        st.session_state.config['email_settings'] = {"default_to": new_to, "default_subject": new_subj, "default_body": new_body}
        save_config(st.session_state.config)
        st.success("Uložené.")
    ...
    if isinstance(prio, list):
        ...
        if st.button("💾 Uložiť"):
            ...
            save_config(st.session_state.config)
            st.success("Uložené")
    else: st.warning("Komplexné priority. Upravte v JSON.")
